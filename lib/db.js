...
function validateUser(userid){
    ...
    exports.db.collection(COLLECTION_LIST.user).findOne(userobj, {_id : true}, function(err, result){
        if(err){
...
function _validatePermission(doc, user, operation){
    ...
    if(owner && owner[user]){
        ...
        tmp[user] = 1;
        groupQuery.push(tmp);
        exports.db.collection('group').find({$or:groupQuery}, {_id:true}).toArray(function(err, item){
            if(err){
                defer.reject(err);
            }else if(item){
                var ret = false;
...
function validatePermission(collection, query, user, operation){
    var defer = Q.defer();
    exports.db.collection(collection).find(query, {permission:true}).toArray(function(err, item){
        if(err){
            defer.reject(err);
...
exports.startDb = function(port, db){
    console.log("start mongodb on port " + port);
    exports.db = mongo.db('localhost:' + port + '/' + db, {safe:true})
};
...
exports.getPermission = function(owner, group, mode, callback){
    ...
    if(owner){
        ...
        if(group){
            ...
            exports.db.collection(COLLECTION_LIST.group).find({$or:groupQuery}, {_id:true}).toArray(function(err, result){
                if(err){
                    callback(err);
                }else if(!result){
                    callback(null, permission);
        ...
        if(!doc.permission){
            exports.getPermission(userid, null, null, function(err, result){
                if(err){
                    ...
                    exports.db.collection(collection).insert(doc, options, callback);
                }
            });
        }else{
            exports.db.collection(collection).insert(doc, options, callback);
    ...
    validateUser(userid).
        then(function(){
            validatePermission(collection, query, userid, OPERATION_CODE.remove).
                then(function(){
                    exports.db.collection(collection).remove(query, options,callback);
                }, function(err){
                    callback(err);
                })
        }).
        ...
        then(function(){
            validatePermission(collection, query, userid, OPERATION_CODE.update).
                then(function(){
                    ...
                    exports.db.collection(collection).update(query, update, options, callback);
                }, function(err){
                    callback(err);
                })
        }).
    ...
    validateUser(userid).
        then(function(){
            validatePermission(collection, query, userid, OPERATION_CODE.find).
                then(function(){
                    exports.db.collection(collection).find(query, fields, options, callback);
                }, function(err){
                    callback(err);
                })
        }).
...
exports.findOne = function(collection, userid, query, callback){
    ...
    validateUser(userid).
        then(function(){
            validatePermission(collection, query, userid, OPERATION_CODE.find)
                .then(function(){
                    exports.db.collection(collection).findOne(query, callback);
                }, function(err){
                    callback(err);
                }
            )}).
...
exports.GridStore = function(filename, mode, options){
    return new GridStore(exports.db, filename, mode, options);
};

exports.write = function(filename, mode, options, data, callback){
    exports.db.gridfs().open(filename, mode, options, function(err, gs){
        if(err){
            callback(err);
        }else{
...
exports.writeFile = function(filename, options, file, callback){
    exports.db.gridfs().open(filename, "w", options, function(err, gs){
        if(err){
            callback(err);
        }else{
...
exports.read = function(filename, options, callback){
    exports.db.gridfs().open(filename, "r", options, function(err, gs){
        if(err){
            callback(err);
        }else{
            gs.read(callback);
...
exports.unlink = function(filename, callback){
    exports.db.gridfs().unlink(filename, callback);
};

exports.exist = function(filename, callback){
    exports.db.gridfs().exist(filename, COLLECTION_LIST.fs, callback);
};
