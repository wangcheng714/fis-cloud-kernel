...
var works = [],
    ...
    serverInfoFile = __dirname + "/../logs/server.log",
...
function writeServerInfo(port){
    fis.log.debug("server pid = " + process.pid);
    var serverInfo = {
        pid : process.pid,
        port : port
    }
    fis.util.write(serverInfoFile, JSON.stringify(serverInfo));
}
...
exports.stop = function(){
    var serverInfo = fis.util.readJSON(serverInfoFile);
    if(serverInfo.pid){
        process.kill(serverInfo.pid, 'SIGINT'); //windows do not support SIGIUP
    }else{
        fis.log.error("do not find the server pid");
    }
}
